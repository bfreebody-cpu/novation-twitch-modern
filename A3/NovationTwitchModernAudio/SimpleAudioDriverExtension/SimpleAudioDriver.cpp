...
{
	bool success = false;
	auto usb_interface = OSDynamicCast(IOUSBHostInterface, in_provider);
	auto device_uid = OSSharedPtr(OSString::withCString("com.twitchmodern.NovationTwitchModernAudio.device"), OSNoRetain);
	auto model_uid = OSSharedPtr(OSString::withCString("Novation-Twitch-0018"), OSNoRetain);
	auto manufacturer_uid = OSSharedPtr(OSString::withCString("Focusrite A.E. Ltd"), OSNoRetain);
	auto device_name = OSSharedPtr(OSString::withCString("Novation Twitch Modern Audio"), OSNoRetain);

	kern_return_t error = kIOReturnBadArgument;
	FailIfNULL(usb_interface, , Failure, "provider is not IOUSBHostInterface");
	...
	ivars->m_simple_audio_device->SetUSBInterface(usb_interface);
